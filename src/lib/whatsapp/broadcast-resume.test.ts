...
describe(
  ...
  () => {
    it(
      'claims when the conditional UPDATE matched a row',
      async () => {
    ...
    it(
      'refuses when another pass already holds the lock',
      async () => {
        const ok =
          await claimBroadcastDelivery(
            claimDb([], []),
    ...
    it(
      'treats a lock older than the staleness window as abandoned',
      async () => {
        const calls: ClaimCall[] = [];
    ...
    it(
      'is scoped to the account, so another tenant cannot claim it',
      async () => {
        const calls: ClaimCall[] = [];
...
describe(
  ...
  () => {
    it(
      'plans the outstanding recipients with their frozen params',
      async () => {
    ...
    it(
      'scopes to failed rows when retrying, and to both for "all"',
      async () => {
    ...
    it(
      'treats a missing or malformed params column as no params',
      async () => {
        const {
          plan,
        } =
    ...
    it(
      'fails unsendable rows up front so they stop blocking the status',
      async () => {
        const writes: PlanWrites =
          {};
    ...
    it(
      'caps one pass and reports the leftover',
      async () => {
        const many =
          Array.from(
            {
    ...
    it(
      '404s a broadcast that is not on this account',
      async () => {
        await expect(
          planBroadcastResume(
            planDb({
    ...
    it(
      'refuses when there is nothing outstanding',
      async () => {
        await expect(
          planBroadcastResume(
            planDb({
    ...
    it(
      'resolves the template row for header + button components',
      async () => {
        const {
          plan,
        } =
